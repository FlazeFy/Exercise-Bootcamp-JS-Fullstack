...
const MoleculesArticleBox: React.FunctionComponent<ArticleData & ExtraProps> = ({objectId, image, title, content, isNavigate, created}) => {
    const CardContent = (
        <div className={`border rounded-xl p-4 bg-gray-50 shadow-inner ${isNavigate && 'cursor-pointer'}`}>
            <div className="relative w-full h-100 mb-4">
                <Image src={image ?? ""} alt={title ?? "Article image"} fill className="rounded-xl object-cover" unoptimized/>
            </div>
    ...
    return isNavigate ? (
        <Link href={`/articles/${objectId}`} className="block">
            {CardContent}
        </Link>
    ) : (
        CardContent
...
export default MoleculesArticleBox;
